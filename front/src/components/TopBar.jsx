...
const TopBar = () => {
  ...
  const getPageTitle = (pathname) => {
    if (pathname.startsWith('/game')) return 'GAME ARENA';
  ...
  const getUserStatus = (targetUserId) => {
     if (friends.some(f => f && f.id === targetUserId)) return 'friend';
     if (sentRequests.some(r => r && r.receiverId === targetUserId)) return 'sent';
     if (pendingRequests.some(r => r && r.senderId === targetUserId)) return 'received';
     return 'none';
  };
          ...
          {showNotifications && (
            <div className="absolute top-14 right-0 w-80 rounded-2xl bg-[#0b1021]/90 backdrop-blur-2xl border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="px-5 py-4 border-b border-white/5 flex justify-between items-center">
                ...
                {totalNotifications > 0 && <span className="text-[10px] bg-red-500/10 text-red-400 px-2 py-0.5 rounded-full border border-red-500/20">{totalNotifications} New</span>}
              </div>
                ...
                {totalNotifications === 0 ? (
                  ...
                  <>
                    ...
                    {pendingRequests.map((req) => (
                      <div key={req.id} className="p-3 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 flex items-center gap-3">
                        <img src={req.sender?.avatar || "/default-avatar.png"} alt={req.sender?.username} className="w-10 h-10 rounded-full object-cover bg-black border border-white/10"/>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-white truncate">{req.sender?.username}</p>
                          <p className="text-[10px] text-white/50">Sent friend request</p>
                        </div>
                        <div className="flex gap-1">
                          <button onClick={() => acceptFriendRequest(req.id)} className="p-2 rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white"><Check size={14} /></button>
                          <button onClick={() => declineFriendRequest(req.id)} className="p-2 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white"><X size={14} /></button>
                        </div>
                      </div>
                    ))}
                  </>
        ...
        {user ? (
          ...
          >
            <div className="text-right hidden md:block">
              ...
              <div className="text-[10px] text-cyan-400 font-mono uppercase">Level {user.level || 1}</div>
            </div>
