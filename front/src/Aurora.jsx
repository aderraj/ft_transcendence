...
void main() {
    ...
    rd.yz *= mm2(0.3); // Fixed tilt
    
    vec3 col = vec3(0.);
    
    // --- LIGHTS FADE ---
    // This creates a safe zone at the horizon where NO lights (aurora/stars) exist.
    // They fade out smoothly before touching y=0.
    float lightsFade = smoothstep(0.0, 0.05, abs(rd.y));
    
    if (rd.y > 0.){
        // --- SKY ---
        col = bg(rd); // Sky Background
        
        // Add Aurora (masked by horizon fade)
        vec4 aur = smoothstep(0.,1.5,aurora(ro,rd)) * lightsFade;
        
        // Add Stars (masked by horizon fade)
        ...
        // Water Gradient: 
        // At y=0 (horizon), multiplier is 1.0 -> Water Color == Sky Color
        // As y increases (deep water), multiplier drops to 0.4 -> Darker
        float waterDarken = smoothstep(0.0, 0.6, rd.y);
        col = bg(rd) * mix(1.0, 0.4, waterDarken);
        
        // Aurora Reflection (masked)
        vec4 aur = smoothstep(0.0,2.5,aurora(ro,rd)) * lightsFade;
        
        // Star Reflection (masked)
...
export default function AuroraOGL({ speed = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        uTime: { value: 0 },
        uResolution: { value: new Vec2(1920, 1080) },
      },
    });
