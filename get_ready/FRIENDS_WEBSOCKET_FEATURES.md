...
friendsSocket.on('friend:request_received', (data) => {
  ...
  // Show notification in UI with accept/decline buttons
});

// Accept friend request (using requestId from the notification)
friendsSocket.emit('friend:accept_request', { requestId: data.requestId }, (response) => {
  if (response.success) {
